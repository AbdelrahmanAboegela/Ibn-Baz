...
export default function SpeechDetailPage() {
    ...
    useEffect(() => {
        api.speeches.get(id)
            .then(setSpeech)
            .catch(() => setError("لم يتم العثور على الخطبة"))
    ...
    return (
        <div className="min-h-screen" dir="rtl">
            <div className="border-b border-border/40 bg-card/30">
                <div className="container mx-auto px-4 py-8 max-w-3xl">
                    ...
                    <div className="flex items-center gap-2 mb-1">
                        ...
                        <span className="text-sm text-muted-foreground">خطبة</span>
                        {speech.date && <span className="text-sm text-muted-foreground">• {speech.date}</span>}
                    </div>
                    <h1 className="text-3xl font-bold font-[family-name:var(--font-amiri)] leading-relaxed mb-4">
                        {speech.title}
                    </h1>
                    {speech.categories?.length > 0 && (
                        <div className="flex gap-2 flex-wrap">
                            {speech.categories.map((c: string) => (
                                <Badge key={c} variant="outline" className="border-amber-600/30 text-amber-400/80 text-xs">
                                    {c}
                                </Badge>
                            ))}
                        </div>
            ...
            <div className="container mx-auto px-4 py-8 max-w-3xl">
                ...
                {/* Hadith Citations — extracted from speech text */}
                {speech.hadith_citations?.length > 0 && (
                    <div className="mt-6 p-5 rounded-xl border border-amber-600/20 bg-amber-950/10">
                        <HadithBlock citations={speech.hadith_citations} />
                    </div>
                )}
